...
<?php 
 if(isset($_POST['Edit']))
 {
  ...
  if(!empty($hotelkode)){
   ...
   $hotelalamat = $_POST['alamat'];
   $hotelketerangan = $_POST['hotelket'];
  ...
  if(empty($nama)){
  	mysqli_query($connection, "UPDATE hotel SET hotelnama= '$hotelnama' , hotelalamat = '$hotelalamat' , hotelketerangan = '$hotelketerangan' , areaID = '$kodearea' WHERE hotelID = '$hotelkode' ");
  ...
  if(($ekstensifile == "jpg") or ($ekstensifile == "JPG"))
  {
    ...
    mysqli_query($connection, "UPDATE hotel SET hotelnama= '$hotelnama' , hotelalamat = '$hotelalamat' , hotelketerangan = '$hotelketerangan' , hotelfoto='$nama' ,areaID = '$kodearea' WHERE hotelID = '$hotelkode' ");
    header("location:hotel.php");
  }
}
}
 ...
 $kodehotel = $_GET["ubah"];
 $editfoto = mysqli_query($connection, "select * from hotel where hotelID = '$kodehotel'");
 $rowedit = mysqli_fetch_array($editfoto);

 $editarea = mysqli_query($connection, "SELECT * FROM hotel, area where hotelID = '$kodehotel' AND hotel.areaID = area.areaID");
...
<form method="POST" enctype="multipart/form-data">
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="alamat" name="alamat" value="<?php echo $rowedit["hotelalamat"]?>">
    </div>
  </div>
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="hotelket" name="hotelket" value="<?php echo $rowedit["hotelketerangan"]?>">
    </div>
  </div>
  ...
  <div class="form-group row">
    <label for="kodearea" class="col-sm-2 col-form-label">Area Hotel</label>
    <div class="col-sm-10">
...
<!-- memulai dengan menampilkan data -->
  <div class="row">
    ...
    if (isset($_POST['kirim'])) {
      $search = $_POST['Search'];
      $query = mysqli_query($connection, "select hotel.* , area.areaID , area.areaNama
        from hotel , area
      ...
      while ($row = mysqli_fetch_array($query)) { ?>
        ...
        <td>
        <a href="hotelhapus.php?hapus=<?php echo $row["hotelID"]?>" class="btn btn-success btn-sm" title="DELETE">
